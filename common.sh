...
schema_setup(){
    ...
    print_head "load schema"
    mysql -h mysql.devopsb71services.site -uroot -p{mysql_root_password} < /app/schema/shipping.sql &>>${log_file}
    status_check $?
  fi
  }
...
java(){
  print_head "Install Maven ${component}"
  yum install maven -y  &>>${log_file}
  status_check $?

  app_prereq_setup

  print_head "download maven dependencies & packaging"
...
python(){
  print_head "Install python ${component}"
